...
@Slf4j
@Component
@ServerEndpoint(path = "/webSocket/ph/{deviceId}")
public class PhWebSocketService {
    /**
     ...
     */
    private static ConcurrentHashMap<Long, PhWebSocketService> webSocketMap = new ConcurrentHashMap<>();
    /**
     ...
     * @param ph
     */
    public static void sendInfo(Ph ph) {
        Long deviceId = ph.getDeviceId();
        //判断当前设备显示端是否已经建立连接
        if (deviceId != null && webSocketMap.containsKey(deviceId)) {
            //从Map中获取当前设备id对应的TempHumWebSocketService对象
            PhWebSocketService coWebSocketService = webSocketMap.get(deviceId);
            //将消息对象转成json字符串
            String msg = JSON.toJSONString(ph);
    ...
    public static synchronized void addOnlineCount() {
        PhWebSocketService.onlineCount++;
    }
