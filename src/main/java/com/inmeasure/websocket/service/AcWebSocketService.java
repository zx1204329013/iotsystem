...
@Slf4j
@Component
@ServerEndpoint(path = "/webSocket/ac/{deviceId}")
public class AcWebSocketService {
    /**
     ...
     */
    private static ConcurrentHashMap<Long, AcWebSocketService> webSocketMap = new ConcurrentHashMap<>();
    /**
     ...
     * @param ac
     */
    public static void sendInfo(Ac ac) {
        Long deviceId = ac.getDeviceId();
        //判断当前设备显示端是否已经建立连接
        if (deviceId != null && webSocketMap.containsKey(deviceId)) {
            //从Map中获取当前设备id对应的TempHumWebSocketService对象
            AcWebSocketService coWebSocketService = webSocketMap.get(deviceId);
            //将消息对象转成json字符串
            String msg = JSON.toJSONString(ac);
    ...
    public static synchronized void addOnlineCount() {
        AcWebSocketService.onlineCount++;
    }
