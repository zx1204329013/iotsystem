...
@Slf4j
@Component
@ServerEndpoint(path = "/webSocket/ws/{deviceId}")
public class WsWebSocketService {
    /**
     ...
     */
    private static ConcurrentHashMap<Long, WsWebSocketService> webSocketMap = new ConcurrentHashMap<>();
    /**
     ...
     * @param ws
     */
    public static void sendInfo(Ws ws) {
        Long deviceId = ws.getDeviceId();
        //判断当前设备显示端是否已经建立连接
        if (deviceId != null && webSocketMap.containsKey(deviceId)) {
            //从Map中获取当前设备id对应的TempHumWebSocketService对象
            WsWebSocketService coWebSocketService = webSocketMap.get(deviceId);
            //将消息对象转成json字符串
            String msg = JSON.toJSONString(ws);
    ...
    public static synchronized void addOnlineCount() {
        WsWebSocketService.onlineCount++;
    }
