...
@Slf4j
@Component
@ServerEndpoint(path = "/webSocket/co2AndVoc/{deviceId}")
public class Co2VocWebSocketService {
    /**
     ...
     */
    private static ConcurrentHashMap<Long, Co2VocWebSocketService> webSocketMap = new ConcurrentHashMap<>();
    /**
     ...
     * @param co2AndVoc
     */
    public static void sendInfo(Co2AndVoc co2AndVoc) {
        Long deviceId = co2AndVoc.getDeviceId();
        //判断当前设备显示端是否已经建立连接
        if (deviceId != null && webSocketMap.containsKey(deviceId)) {
            //从Map中获取当前设备id对应的TempHumWebSocketService对象
            Co2VocWebSocketService coWebSocketService = webSocketMap.get(deviceId);
            //将消息对象转成json字符串
            String msg = JSON.toJSONString(co2AndVoc);
    ...
    public static synchronized void addOnlineCount() {
        Co2VocWebSocketService.onlineCount++;
    }
